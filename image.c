...
/* ---------- Fonction qui met en gris ---------- */
void image_en_gris(Image *img) {
    for (int i = 0; i < img->width * img->height; i++) {
        int moyenne = (img->data[i].r + img->data[i].g + img->data[i].b) / 3;
        img->data[i].r = img->data[i].g = img->data[i].b = moyenne;
    }
}
